  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white font-bold text-lg">M</div>
...
export default function App() {
  ...
  const { fetchCategories, fetchNotifications } = useAppStore()
  ...
  useEffect(() => {
    ...
    // Listen for auth changes (e.g. user accepts invite, password reset)
  ...
  // Load shared data when user logs in
  useEffect(() => {
    if (user) {
      fetchCategories()
    ...
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path="/users" element={
          <ProtectedRoute allowedRoles={['ceo', 'manager']}><UsersPage /></ProtectedRoute>
        } />
        <Route path="/notifications" element={
          <ProtectedRoute><NotificationsPage /></ProtectedRoute>
        } />
        <Route path="/home" element={
          <ProtectedRoute allowedRoles={['site_manager', 'site_worker']}><HomeDispatch /></ProtectedRoute>
        } />
